...
class GUI:
    def __init__(self):
        ...
        # BACKGROUND IMAGE
        self.bgd_path = rospy.get_param("~bgd_path", None)
        self.bgd_name = rospy.get_param("~bgd_name", "bgd_map.png")
        self.coords_name = rospy.get_param("~coords_name", "buchlovice.mapdata")
        ...
        # QR CODE
        self.save_path = rospy.get_param("~save_path", None)
        self.qr_scale = 8
    ...
    def load_map_data(self, file_name):
        ...
        with open(self.bgd_path + file_name, "rb") as fh:
    ...
    def create_qr(self, geo):
        ...
        return qr.png(self.save_path + f"qr{self.counter}.png", scale=self.qr_scale)


def main():
    rospy.init_node("qr_qui")
    window = GUI()
